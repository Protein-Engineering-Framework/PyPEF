...
----------------
    ...
    -------------------
        ...
        PyPEF provides three encoding options for training machine learning models, namely
        supervised regression models:

            1. DCA: Based on couplings.param file (generated by the C framework PLMC_FILE).

            2. AAidx: Based on AAindex descriptors (566 amino acid descriptor files
                       taken from the AAindex database).

            3. OneHot: Simple one-hot encoding representing the occurrence of an
                       amino acid at a sequence position as a single 1 and 19 0's.

        Further, DCA-based encoding allows a pure ML-based modeling as well as a hybrid
        ...
        Encoding option 2 is an alternative encoding and modeling technique if no MSA
        can be constructed which is required for encoding option 1. Further, one-hot
        encoding (encoding option 3) provides a simple but fast and often well-performing
        encoding option that will likely outperform the AAindex-based encoding technique for
        model generalization.


    II. Hybrid modeling
    -------------------
        A hybrid model that combines pure statistical DCA-based prediction (a variant's relative
        'evolutionary energy' to wild-type) and DCA-encoding based training of a ML model similar
        to pure ML modeling option I.1.
        Based on features generated from the direct coupling analysis (.params file output by PLMC).
        Individual model contributions are optimization only based on Spearman's correlation
        coefficient and thus, only variant fitness ranks are to be considered for evaluating model
        performance, not the exact predicted fitness value. For regression, only L2-regularized linear
        regression (Ridge regression) is provided as modeling option.


Running example of pure ML model
--------------------------------
...
indices taken from the AAIndex database).
 1. To train and validate models run
        pypef aaidx -l Learning_Set.fasta -t TEST_Set.fasta [--threads THREADS]
...
Creation of prediction sets: To create prediction sets from CSV data (using single-
substituted variant data)

        pypef mkps --wt WT_SEQ --input CSV_FILE
        [--drop THRESHOLD] [--drecomb] [--trecomb] [--qarecomb] [--qirecomb]
        [--ddiverse] [--tdiverse] [--qdiverse]

Encoding a CSV file (for further protein engineering performance studies such as
"low N" or "mutational extrapolation"
...
Usage:
    ...
    pypef ml --encoding ENCODING_TECHNIQUE --model MODEL12345 --ps PREDICTION_SET
        [--params PLMC_FILE] [--nofft] [--negative]
